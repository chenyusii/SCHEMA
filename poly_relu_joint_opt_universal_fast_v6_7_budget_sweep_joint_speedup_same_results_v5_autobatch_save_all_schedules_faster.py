...
Compared to the previous "fast" script, this v2 adds **more speedups** for large nets like ResNet56:
...
# WEIGHTS_REL = Path("./examples/resnet/gen_aespa_weights/ResNet20_cifar100.pt")
OUTPUT_DIR_REL = Path("examples/resnet/gen_aespa_weights/poly_opt_outputs_fast_v2_res32_cifar10_joint")
...
ENABLE_MIXED_SCALING_IN_DP = True
...
TOTAL_BUDGET = 98  # for ResNet56 typical; adjust if desired



# -------- Sweep mode --------
# "dp_joint": sweep DP then JOINT (uniform disabled in sweep)
# "dp_uniform": sweep DP then UNIFORM
SWEEP_MODE = "dp_joint"

# JOINT sweep controls (used when SWEEP_MODE=="dp_joint")
JOINT_SWEEP_MAX_STEPS = 40
JOINT_SWEEP_MOVE_TRIALS_PER_STEP = 256
JOINT_SWEEP_TOPK_VAL = 8
BUDGET_SWEEP_JSON_NAME_DP_JOINT = "budget_sweep_dp_joint_metrics.json"
...
def main() -> None:
    ...
    if per_site_error is None or per_site_best_scale is None:
        ...
        for si, s in enumerate(sites, start=1):
            ...
            for di, d in enumerate(cand_degs, start=1):
                ...
                ev_min = eval_model(
                    model_fx, sweep_loader, device, sites, used_bounds_ref,
    ...
    # -------------------------------------------------------------------------
    # Budget sweep (VAL selection only):
    #   - SWEEP_MODE="dp_joint" : DP -> JOINT (uniform disabled in sweep)
    #   - SWEEP_MODE="dp_uniform": DP -> UNIFORM (legacy)
    #
    # IMPORTANT: decisions are on VAL; TEST is report-only.
    # -------------------------------------------------------------------------
    if ENABLE_BUDGET_SWEEP:
        import json as _json

        mode = str(SWEEP_MODE).lower().strip()
        if mode not in ("dp_joint", "dp_uniform"):
        ...
        # cost buckets for cost-neutral moves in JOINT
        cost_buckets = build_cost_buckets(CANDIDATE_DEGREES)

        # Joint sweep hyperparams
        _joint_steps = int(min(JOINT_MAX_STEPS, JOINT_SWEEP_MAX_STEPS))
        _joint_trials = int(JOINT_SWEEP_MOVE_TRIALS_PER_STEP)
        _joint_topk_val = int(JOINT_SWEEP_TOPK_VAL)

        results: List[Dict[str, Any]] = []

        for budget in budgets:
            print("\n" + "=" * 120)
            print(f"[sweep] mode={mode} TOTAL_BUDGET={budget}")
            print("=" * 120)
            ...
            # ---------------------------------------------------------------
            # Save per-budget DP schedule bounds table on CALIB
            # (ref bounds + used bounds + observed min/max + t/oor)
            # NOTE: this does NOT affect search decisions (report-only).
            #
            # Speed: in dp_joint mode, we reuse JOINT's initial FULL(CALIB) stats
            # for the DP-start schedule, so we skip an extra CALIB pass here.
            # ---------------------------------------------------------------
            if mode != "dp_joint":
    # ---------------------------------------------------------------
                # Save per-budget DP schedule bounds table on CALIB
                ...
                # ---------------------------------------------------------------
                try:
                    sweep_sched_dir = out_dir / "sweep_schedules" / f"budget{int(budget):04d}"
                    ev_dp_calib = eval_model(
                        model_fx, calib_loader, device, sites, used_bounds_ref,
                        baseline_logits_cpu=None, collect_stats=True
                    )
                    dp_calib_rows: List[Dict[str, Any]] = []
                    for s in sites:
                        deg = int(dp_schedule.get(s, 0))
                        ref_a, ref_b = ref_bounds.get(s, (float("nan"), float("nan")))
                        used_a, used_b = used_bounds_ref.get(s, (float("nan"), float("nan")))
                        st = ev_dp_calib.stats.get(s) if (ev_dp_calib.stats is not None) else None
                        dp_calib_rows.append({
                            "site": s,
                            "deg": deg,
                            "ref_a": float(ref_a),
                            "ref_b": float(ref_b),
                            "used_a": float(used_a),
                            "used_b": float(used_b),
                            "min_obs": float(st.min_obs) if st is not None else float("nan"),
                            "max_obs": float(st.max_obs) if st is not None else float("nan"),
                    ...
                    save_final_schedule_bounds_table(
                        dp_calib_rows,
                        txt_path=sweep_sched_dir / "dp_final_schedule_bounds_calib.txt",
                        csv_path=sweep_sched_dir / "dp_final_schedule_bounds_calib.csv",
                        title=f"FINAL schedule (DP) on CALIB | budget={int(budget)} | cost={int(dp_cost)}/{int(budget)}",
                    )
                except Exception as _e:
                    print(f"[warn] failed to save DP per-budget FINAL schedule bounds table: {_e}")

            
            row: Dict[str, Any] = {
                "total_budget": int(budget),
                "dp": {"cost": int(dp_cost), "val_obj": float(dp_val_obj), "val_acc": float(dp_val_acc)},
            }
            ...
            # --------------------------
            if mode == "dp_joint":
                old_trials = int(JOINT_MOVE_TRIALS_PER_STEP)
                old_topk = int(JOINT_TOPK_VAL)
                try:
                    globals()["JOINT_MOVE_TRIALS_PER_STEP"] = _joint_trials
                    globals()["JOINT_TOPK_VAL"] = _joint_topk_val

                    print(f"[joint-sweep] joint_search: steps={_joint_steps}, trials/step={_joint_trials}, topk_val={_joint_topk_val}")
                    joint_schedule, ev_full_cur, ev_full_init, _full_sched_at_full = joint_search(
                        model_fx,
                        device,
                        sites,
                        schedule_ref,
                    ...
                    globals()["JOINT_MOVE_TRIALS_PER_STEP"] = old_trials
                    globals()["JOINT_TOPK_VAL"] = old_topk


                # ---------------------------------------------------------------
                # Save per-budget DP-start schedule bounds table on CALIB
                # Reuse JOINT's initial FULL(CALIB) stats computed at the DP-start schedule.
                # This avoids an extra CALIB pass and does NOT affect search decisions.
                # ---------------------------------------------------------------
                try:
                    sweep_sched_dir = out_dir / "sweep_schedules" / f"budget{int(budget):04d}"
                    dp_calib_rows: List[Dict[str, Any]] = []
                    for s in sites:
                        ...
                        ref_a, ref_b = ref_bounds.get(s, (float("nan"), float("nan")))
                        used_a, used_b = used_bounds_ref.get(s, (float("nan"), float("nan")))
                        st = ev_full_init.stats.get(s) if (ev_full_init.stats is not None) else None
                        dp_calib_rows.append({
                            "site": s,
                            "deg": deg,
                            "ref_a": float(ref_a),
                    ...
                    save_final_schedule_bounds_table(
                        ...
                        txt_path=sweep_sched_dir / "dp_final_schedule_bounds_calib.txt",
                        csv_path=sweep_sched_dir / "dp_final_schedule_bounds_calib.csv",
                        title=f"FINAL schedule (DP-start) on CALIB | budget={int(budget)} | cost={int(dp_cost)}/{int(budget)}",
                    )
                except Exception as _e:
                    print(f"[warn] failed to save DP-start per-budget FINAL schedule bounds table (reuse full-init): {_e}")


                # Explicit VAL print for JOINT (requested)
                apply_schedule_and_scaling(
                    model_fx=model_fx,
                    sites=sites,
                    schedule_ref=schedule_ref,
                ...
                ev_joint_val = eval_model(
                    model_fx, val_loader, device, sites, used_bounds_ref,
                    baseline_logits_cpu=base_val_logits, collect_stats=True
                )
                pen_joint_val = penalize_and_guard(ev_joint_val, ref_bounds)
                joint_val_obj = float(pen_joint_val.obj)
                joint_val_acc = float(ev_joint_val.acc)
                print(f"[joint] VAL: obj={joint_val_obj:.6e} acc={_fmt_pct(joint_val_acc)}")

                # ---------------------------------------------------------------
                # Save per-budget JOINT schedule bounds table on CALIB
                # (ref bounds + used bounds + observed min/max + t/oor)
                # NOTE: this does NOT affect search decisions (report-only).
                # ---------------------------------------------------------------
                try:
                    sweep_sched_dir = out_dir / "sweep_schedules" / f"budget{int(budget):04d}"
                    ev_joint_calib = eval_model(
                        model_fx, calib_loader, device, sites, used_bounds_ref,
                        baseline_logits_cpu=None, collect_stats=True
                    )
                    joint_calib_rows: List[Dict[str, Any]] = []
                    for s in sites:
                        deg = int(joint_schedule.get(s, 0))
                        ref_a, ref_b = ref_bounds.get(s, (float("nan"), float("nan")))
                        used_a, used_b = used_bounds_ref.get(s, (float("nan"), float("nan")))
                        st = ev_joint_calib.stats.get(s) if (ev_joint_calib.stats is not None) else None
                        joint_calib_rows.append({
                            "site": s,
                            "deg": deg,
                            "ref_a": float(ref_a),
                            "ref_b": float(ref_b),
                    ...
                    save_final_schedule_bounds_table(
                        joint_calib_rows,
                        txt_path=sweep_sched_dir / "joint_final_schedule_bounds_calib.txt",
                        csv_path=sweep_sched_dir / "joint_final_schedule_bounds_calib.csv",
                        title=f"FINAL schedule (JOINT) on CALIB | budget={int(budget)} | cost={int(schedule_cost(joint_schedule))}/{int(budget)}",
                    )
                except Exception as _e:
                    print(f"[warn] failed to save JOINT per-budget FINAL schedule bounds table: {_e}")

                row["joint"] = {
                    "cost": int(schedule_cost(joint_schedule)),
                    "val_obj": float(joint_val_obj),
                    "val_acc": float(joint_val_acc),
                    "steps": int(_joint_steps),
                    "trials_per_step": int(_joint_trials),
                    "topk_val": int(_joint_topk_val),
                }

                # Report-only TEST full after joint (optional)
                eval_on_test_full(
                    stage=f"sweep_joint_budget{budget}",
                    model=model_fx,
                    loader=test_loader,
                    device=device,
                    sites=sites,
            ...
            # --------------------------
            # Legacy DP-uniform sweep (kept for compatibility)
            # --------------------------
            if mode == "dp_uniform":
                ...
                for ud in uni_deg_list:
                    ...
                    rec = {"deg": int(ud), "cost": int(uni_cost), "val_obj": float(pen_u_val.obj), "val_acc": float(ev_u_val.acc)}
                ...
                row["uniform_best"] = best_uniform
                if SAVE_ALL_UNIFORM_PER_BUDGET:
                    row["uniform_all"] = uniform_all

            results.append(row)

        # Save JSON
        out_json = (out_dir / BUDGET_SWEEP_JSON_NAME_DP_JOINT) if mode == "dp_joint" else (out_dir / BUDGET_SWEEP_JSON_NAME)
        payload = {
            "meta": {
                ...
                "joint_sweep_steps": int(_joint_steps) if mode == "dp_joint" else None,
                "joint_sweep_trials_per_step": int(_joint_trials) if mode == "dp_joint" else None,
                "joint_sweep_topk_val": int(_joint_topk_val) if mode == "dp_joint" else None,
            },
            "rows": results,
        }
        with out_json.open("w", encoding="utf-8") as f:
            _json.dump(payload, f, ensure_ascii=False, indent=2)
        print(f"[json] saved sweep results to: {out_json}")
        return
        # return
    # DP schedule
    ...
    joint_schedule, ev_full_cur, _ev_full_init, _full_sched_at_full = joint_search(
